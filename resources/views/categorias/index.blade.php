<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Categorías</h2>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-900/40">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">Descripción</th>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700/60">
                        @forelse ($categorias as $categoria)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors">
                                <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-gray-100">{{ $categoria->nombre }}</td>
                                <td class="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">{{ $categoria->descripcion ?? '—' }}</td>
                                <td class="px-6 py-4 text-right text-sm whitespace-nowrap">
                                    <a href="{{ route('categoria.edit', $categoria) }}" class="font-medium text-accent-700 dark:text-accent-400 hover:text-accent-800 dark:hover:text-accent-300 transition-colors">Editar</a>
                                    <form action="{{ route('categoria.destroy', $categoria) }}" method="POST" class="inline ml-3" onsubmit="return confirm('¿Eliminar esta categoría?')">
                                        @csrf @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-6 py-12 text-center text-sm text-gray-500 dark:text-gray-400">
                                    ...
                                    <a href="{{ route('categoria.create') }}" class="text-accent-700 dark:text-accent-400 hover:underline ml-1">Crea la primera.</a>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
