<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('cliente.index') }}" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors" aria-label="Volver">
            ...
            <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Nuevo cliente</h2>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
                <form action="{{ route('cliente.store') }}" method="POST" x-data="{ enviando: false }" @submit="enviando = true" class="space-y-5">
                    @csrf
                    ...
                    <div>
                        <label for="telefono" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Teléfono</label>
                        ...
                        @error('telefono') <p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                    </div>

                    <div class="flex justify-end gap-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                        <a href="{{ route('cliente.index') }}" class="px-4 py-2 text-sm font-medium rounded-md border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">Cancelar</a>
                        <button type="submit" :disabled="enviando"
                            ...
                            <span x-text="enviando ? 'Guardando…' : 'Guardar cliente'">Guardar cliente</span>
                        </button>
                    </div>
                </form>
            </div>
